...
def get_soil_moisture_and_temperature(df):
    ...
    for year in tqdm(range(2010, 2020), desc=f"total progress"):
        for month in tqdm(range(1, 13), desc=f"   {year} months"):
            ...
            df = df.apply(get_info, args=(date, info), axis=1)
    df.to_csv("./outbreak_data_plus_info.csv", index=False)
...
if __name__ == "__main__":
    ...
    df = pd.DataFrame(outbreaks[['latitude', 'longitude', 'location_period_id']])
    df = df.drop_duplicates()   # soil moisture and soil temperature
    df = df.apply(round_coordinates, axis=1)
    get_soil_moisture_and_temperature(df)
